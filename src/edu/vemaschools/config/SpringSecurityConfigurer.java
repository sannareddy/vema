	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		auth.jdbcAuthentication()
				...
				.authoritiesByUsernameQuery(
						"SELECT users.username as username,trim(roles.role_name) as role FROM APP_USER_ROLES ur INNER JOIN APP_USERS users ON ur.userid=users.userid INNER JOIN APP_ROLES roles ON ur.roleid=roles.roleid WHERE users.username= ?")
				.dataSource(dataSource);
	}

	@Override
	public void configure(WebSecurity web) throws Exception {
		web.ignoring().antMatchers("/lib/**", "/images/**", "/open/**", "/global/**", "/node/**", "/fonts/**",
				"/js/**");
	}
